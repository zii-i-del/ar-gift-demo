import background from './composition-background.webm?url';
...
export default function Composition(){
 ...
 useEffect(()=>{let disposed=false,frame=0;const video=document.createElement('video');video.muted=true;video.loop=true;video.src=background;
 const r=new GiftRenderer(video),c=new Confetti(),i=new Interaction();video.style.cssText='position:absolute;inset:0;width:100%;height:100%;transform:scaleX(-1)';host.current!.appendChild(video);r.renderer.domElement.style.cssText='position:absolute;inset:0';host.current!.appendChild(r.renderer.domElement);c.width=i.width=640;c.height=i.height=360;
