...
const lost=new Interaction();lost.reset('bubble');
...
const gap=new Interaction();gap.reset('bubble');
...
assert.ok(gap.emittedBubbles>n,'confirmed hand recovers from brief uncertainty without another 250ms');
gap.acceptHands([uncertain],800);gap.step(1/60,800);
gap.acceptHands([good],1000);const after=gap.emittedBubbles;gap.step(1/60,1000);
assert.equal(gap.emittedBubbles,after,'long uncertainty requires fresh confirmation');
const initial=new Interaction();initial.reset('bubble');
