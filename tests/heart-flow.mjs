...
for(const [dx,dy] of [[1,0],[-1,0],[0,-1],[1,-1]]) {
 ...
 const h=hand('L',400,400,dx,dy);e.acceptHands([h],0);e.acceptHands([h],100);
...
assert.ok(e.emittedHearts>45&&max>=10,'dense bounded double-hand stream');
...
for(const engine of [headEngine,handEngine]){engine.reset('bubble');Object.assign(engine.bubbles[0],{active:true,x:320,y:180,r:20,age:1,pop:-1,vx:50,vy:200});}
