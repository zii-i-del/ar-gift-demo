...
h.acceptHands([heart],0);h.acceptHands([heart],100);h.step(0,100);assert.equal(h.emittedHearts,1);
h.acceptHands([{...heart,heart:false,heartPossible:true}],180);assert.equal(h.memories.get('left').heartSince,-1);
h.acceptHands([{...heart,heart:false,heartPossible:true}],340);assert.equal(h.memories.get('left').heartSince,-1);
console.log('PASS: impact, visible travel, bounds, uncertain gesture stops confirmation');
