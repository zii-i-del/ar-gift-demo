...
function run(kind,portrait,two,low=false){
 const e=new Interaction(true);e.reset('auto');e.width=portrait?360:640;e.height=portrait?640:360;e.autoLow=low;const births=[];
...
for(const kind of ['hearts','bubble'])for(const portrait of [false,true])for(const low of [false,true]){
 const a=run(kind,portrait,false,low),b=run(kind,portrait,true,low);assert.deepEqual(a.births,b.births,'second hand cannot change cadence');
 const minimum=kind==='hearts'?350:portrait?(low?240:180):(low?180:125);
 ...
 assert(a.births.length>2);console.log(kind,portrait?'portrait':'landscape',low?'low':'standard',a.births.length);
}
...
const mixed=new Interaction(true);mixed.reset('auto');for(const t of [0,100,200,300]){mixed.acceptHands([hand('A','hearts'),hand('B','bubble')],t);mixed.step(.016,t);}assert(mixed.emittedHearts>0);assert.equal(mixed.emittedBubbles,0);
...
const old=mixed.emittedBubbles;mixed.autoBlocked=true;mixed.step(.016,360);mixed.autoBlocked=false;mixed.step(.016,370);assert.equal(mixed.emittedBubbles,old,'mouth/protection suspension cannot resume cached gesture');mixed.acceptHands([hand('B','bubble')],400);mixed.step(.016,400);assert.equal(mixed.emittedBubbles,old);
