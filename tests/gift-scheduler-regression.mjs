...
function simulate(select,cost){
 ...
 for(let now=0;now<7000;now+=1000/60){budget=Math.min(700,budget+(now-previous)*.7);previous=now;if(now<busy||now<next||Math.floor(now/33.333)===video)continue;
  const r=select(now,'playing',last,cost,new Set(),budget);next=r.nextCaptureAt;if(!r.task)continue;
...
assert.equal(GiftScheduler.select(170,'preparing',last,cost,new Set(),700,0).task,'hair');
assert.equal(GiftScheduler.select(170,'preparing',last,cost,new Set(),0,0).reason,'budget-exhausted');
const slow=simulate(GiftScheduler.select,{hands:35,face:25,hair:60,pose:30});
assert.equal(GiftScheduler.select(170,'preparing',last,{...cost,hair:300,pose:300},new Set(),700).reason,'freshness-reserve');
