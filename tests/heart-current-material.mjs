...
const pink=read('heart-crystal-v21-pink'),peach=read('heart-crystal-v21-yellow');
...
assert.equal(peach.g.accessors[b.indices].count/3,4416);
...
console.log('PASS: peach shares pink geometry/UV/animation data, 4416 triangles, opaque, no transmission');

for(const asset of [pink,peach]){
 assert.equal(asset.g.images.length,1);
 assert.ok(asset.g.images.every(i=>i.bufferView!==undefined),'embedded current texture');
 for(const m of asset.g.materials){
  assert.ok(m.pbrMetallicRoughness.baseColorTexture);
  assert.equal(m.emissiveTexture,undefined);
