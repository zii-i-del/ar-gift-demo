...
const baseline=JSON.parse(readFileSync(new URL('./fixtures/original-spacing.json',import.meta.url)));
...
function run(ids){const e=new Interaction(),births=[];let total=0;
 for(let i=0;i<180;i++){const now=i*1000/60;if(i%4===0){e.acceptHands(ids.map(hand),now);if(e.emittedHearts>total){total=e.emittedHearts;for(const h of e.hearts)if(h.active&&h.age===0)births.push({time:now,...h});}}e.step(1/60,now);assert.ok(e.hearts.filter(h=>h.active).length<=12);}
 return {e,births};}
...
try{
 ...
 assert.deepEqual(single.births.map(b=>b.time),baseline.births.map(b=>b.time));
 for(let i=0;i<single.births.length;i++){const b=single.births[i],old=baseline.births[i];assert.equal(b.x,old.x);assert.equal(b.y,old.y);assert.ok(Math.abs(Math.hypot(old.vx,old.vy)-Math.hypot(b.vx,b.vy))<1e-8);}
 const blocked=new Interaction();blocked.hearts.forEach(h=>Object.assign(h,{active:true,owner:'L',age:1}));blocked.acceptHands([hand('L')],0);blocked.acceptHands([hand('L')],100);assert.equal(blocked.memories.get('L').heartSequence,0,'failed birth never advances sequence');
}finally{Math.random=originalRandom;}
console.log('PASS: per-hand sequence, jitter bounds, original timing/origin/speed, narrow equal-age envelope, successful-birth counter');
