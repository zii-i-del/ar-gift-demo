...
左右手都可用于发射爱心或泡泡。同一时刻由一只主要发射手负责，停止或离开后可交给另一只手，保持稳定的发射节奏。明确改变手势后停止新增，已生成的特效继续运动直至消失
...
三种互动围绕爱意、加油和感动设计，由主播主动触发。虚拟礼物从动作中产生，并通过与脸部、手掌、头发和肩部的接触响应融入直播画面，增强表达与互动的趣味性
...
本地开发、构建和预览使用同一份 Vite 配置。`dev` 和 `build` 在运行前执行 [模型准备脚本](scripts/prepare-vision.mjs)：从已安装的 MediaPipe 包复制运行文件，并下载固定版本的脸、手、头发和肩部模型。`preview` 直接提供已有构建产物。首次准备需要网络；已有模型会复用。运行时浏览器从网站同源加载这些资源，视频帧不发送到模型下载服务器
...
构建输出为 `dist/`，包含网页、特效脚本、图片、模型和 WASM 资源，预览地址以终端输出为准

`build` 会清空输出目录，不能把需要保留的文件或 Git 工作目录放在 `dist/` 内
...
| [pages/](pages/)、[vite.config.ts](vite.config.ts) | 共用的网页入口与开发、构建、预览配置 |

主要技术：React、TypeScript、MediaPipe Tasks Vision、Three.js；本地开发、构建与预览统一使用 Vite，构建产物发布到 GitHub Pages，不需要业务后端或 API Key

### 发布方式

- `main` 保存源码，GitHub Pages 从 `gh-pages` 分支根目录提供网站
- 更新后先构建，再将 `dist/` **目录内的产物**发布到 `gh-pages`；只推送 `main` 不会更新在线页面
