...
import {GiftGpuTimer} from '../lib/gift-performance';
...
export default function Home(){
 ...
 useEffect(()=>{
  if(!running)return;
  const v=video.current!,root=host.current!,id=++session.current,c=new Confetti(),i=new Interaction(true),g=new GiftCoordinator();
  ...
  const createRenderer=async()=>{
    ...
    gpu=new GiftGpuTimer(r.renderer.getContext() as WebGL2RenderingContext);
