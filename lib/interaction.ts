...
export type Hand = {
  ...
  span: number;
  heartOrigin?: Point;
  heartDirection?: Point | null;
  gun?: boolean;
  gunUncertain?: boolean;
  gunPose?: GunPose3D;
  gunReason?: string;
  indexBends?: [number, number];
  gunDirection?: Point;
  heart: boolean;
  heartPossible?: boolean;
  pointing: boolean;
  palm: boolean;
  reach: number;
};
...
const pointSegmentDistance = (p: Point, a: Point, b: Point) => {
  const dx = b.x - a.x,
    dy = b.y - a.y;
  const t = clamp(
    ((p.x - a.x) * dx + (p.y - a.y) * dy) / Math.max(1, dx * dx + dy * dy),
    0,
    1,
  );
...
export function readHand(
  ...
  const extended = (tip: number, pip: number) =>
    distance(p(tip), wrist) > distance(p(pip), wrist) * 1.08;
  ...
  const segmentDistance = (() => {
    ...
    const t = Math.max(
      ...
      Math.min(
        1,
        ((thumb.x - a.x) * dx + (thumb.y - a.y) * dy) /
          Math.max(1, dx * dx + dy * dy),
      ),
    );
    return distance(thumb, { x: a.x + dx * t, y: a.y + dy * t }) / span;
  })();
  const crossed =
    segmentsCross(p(2), thumb, p(5), tip) ||
    segmentsCross(p(1), thumb, p(5), p(8));
  // A bent/occluded index need not be farther from the wrist than its PIP.
  const ix = tip.x - p(6).x,
    iy = tip.y - p(6).y;
  const tx = thumb.x - p(3).x,
    ty = thumb.y - p(3).y;
  const angleSine =
    Math.abs(ix * ty - iy * tx) /
    Math.max(1, Math.hypot(ix, iy) * Math.hypot(tx, ty));
  const foldedCount = [middle, ring, pinky].filter((v) => !v).length;
  const separated = thumbIndex > 0.3 && thumbIndex < 1.8;
  const thumbJoint = p(3),
    indexJoint = p(6);
  const emitter = fingerEmitter(
    p(3),
    thumb,
    p(6),
    tip,
    {
      x: (p(5).x + p(17).x + wrist.x) / 3,
      y: (p(5).y + p(17).y + wrist.y) / 3,
    },
    p(7),
  );
  ...
  const exposedTips =
    emitter.thumbBeyond >= Math.max(span * 0.22, emitter.thumbLength * 0.25) &&
    emitter.indexBeyond >= Math.max(span * 0.22, emitter.indexLength * 0.25);
  const distalContact =
    segmentsCross(thumbJoint, thumb, indexJoint, tip) ||
    Math.min(
      pointSegmentDistance(thumbJoint, indexJoint, tip),
      pointSegmentDistance(thumb, indexJoint, tip),
      pointSegmentDistance(indexJoint, thumbJoint, thumb),
      pointSegmentDistance(tip, thumbJoint, thumb),
    ) <
      span * 0.18;
  const crossEvidence =
    exposedTips &&
  ...
  const thumbProgress =
    ((thumb.x - base.x) * axisX + (thumb.y - base.y) * axisY) / axisLength2;
  const indexChain =
    distance(base, pip) + distance(pip, p(7)) + distance(p(7), tip);
  const clearIndex =
    distance(base, tip) / Math.max(1, indexChain) > 0.88 &&
    distance(base, tip) > span * 0.7 &&
    thumbProgress < 0.42 &&
    thumbIndex > 0.65;
  const heart =
    !clearIndex &&
  ...
  const palmX = base.x - wrist.x,
    palmY = base.y - wrist.y,
    palmLength = Math.max(1, Math.hypot(palmX, palmY));
  const thumbForward =
    ((thumb.x - base.x) * palmX + (thumb.y - base.y) * palmY) / palmLength;
  const thumbLateral =
    Math.abs((thumb.x - base.x) * palmY - (thumb.y - base.y) * palmX) /
    palmLength;
  const thumbOutside = thumbForward > span * 0.12 || thumbLateral > span * 0.65;
  ...
  const gunForward =
    ((tip.x - base.x) * palmX + (tip.y - base.y) * palmY) /
  ...
  const projectedStraight =
    indexRatio > 0.99 &&
    Math.max(bend1, bend2) < 15 &&
    gunForward > 0.7 &&
    gunSeparation > 0.6;
  ...
  const gunDirection = {
    ...
    y: iy / Math.max(1, gunLength),
  };
  const palm =
    !gun &&
    !heart &&
    !pointing &&
    (extendedCount >= 2 || distance(p(4), p(20)) > span * 1.1);
  const tips = [p(8), p(12), p(16), p(20)];
  // Upper envelope of the open fingers is the visible support surface.
  const anchor = {
    x: tips.reduce((n, t) => n + t.x, 0) / 4,
    y: Math.min(...tips.map((t) => t.y)),
  };
  return {
    id,
    landmarks: raw,
    tip,
    heartOrigin: emitter.origin,
    heartDirection: emitter.direction,
    wrist,
    anchor,
    span,
    gun,
    ...
    reach:
      (raw[5 * 3 + 2] - raw[8 * 3 + 2]) /
      Math.max(0.03, Math.hypot(raw[15] - raw[51], raw[16] - raw[52])),
  };
}
...
type HandMemory = {
  ...
  ovx: number;
  ovy: number;
  directionSeen: number;
  heartSequence: number;
};
export type Heart = {
  active: boolean;
  x: number;
  ...
  emitterAngle?: number;
  spreadAngle?: number;
  appearanceAngle?: number;
  birthOrder?: number;
  colorOrder?: number;
  contactAge?: number;
  squashAmount?: number;
  hitAngle?: number;
  motesReleased?: boolean;
  motionAngle?: number;
...
export class Interaction {
  ...
  bubbles: Bubble[] = Array.from({ length: BUBBLE_CAPACITY }, () => ({
    active: false,
    x: 0,
    y: 0,
    vx: 0,
  ...
  acceptHead(head: HeadState | null) {
    this.head = head;
  }
  width = 640;
  height = 400;
  ...
  popped = 0;
  reset() {
    this.primaryEmitter.reset();
    this.autoSuspended = false;
    this.lastHandSample = -Infinity;
    this.lastSharedHeart = this.lastSharedBubble = -Infinity;
    this.lastHeartOwner = '';
    this.memories.clear();
    this.head = null;
  ...
  acceptHands(hands: Hand[], now: number) {
    ...
    hands = [...hands].sort(
      (a, b) =>
        (a.id === this.lastHeartOwner ? 1 : 0) -
          (b.id === this.lastHeartOwner ? 1 : 0) || a.id.localeCompare(b.id),
    );
    ...
    for (const hand of hands) {
      ...
      if (!memory || now - memory.seen > 200) {
        memory = {
          ...
          vy: 0,
          palmBody: null,
          ox: hand.heartOrigin?.x ?? hand.tip.x,
          oy: hand.heartOrigin?.y ?? hand.tip.y,
          dx: 0,
          dy: -1,
          ovx: 0,
          ovy: 0,
          directionSeen: -Infinity,
          heartSequence: 0,
        };
        this.memories.set(hand.id, memory);
      }
      const previous = memory.hand;
      const sampleDt = Math.max(0.016, (now - memory.seen) / 1000);

      const origin = hand.heartOrigin ?? hand.tip;
      const alpha = 1 - Math.exp(-sampleDt * 25),
        oldX = memory.ox,
        oldY = memory.oy;
      memory.ox += (origin.x - memory.ox) * alpha;
      memory.oy += (origin.y - memory.oy) * alpha;
      memory.ovx = clamp((memory.ox - oldX) / sampleDt, -320, 320);
      memory.ovy = clamp((memory.oy - oldY) / sampleDt, -320, 320);
      const direction = hand.heartDirection;
      if (direction && Math.hypot(direction.x, direction.y) > 0.1) {
        let angle = Math.atan2(memory.dy, memory.dx);
        const target = Math.atan2(direction.y, direction.x);
        angle +=
          Math.atan2(Math.sin(target - angle), Math.cos(target - angle)) *
          (memory.directionSeen === -Infinity ? 1 : alpha);
        memory.dx = Math.cos(angle);
        memory.dy = Math.sin(angle);
        memory.directionSeen = now;
      }
      if (hand.palm) {
        const prior = memory.palmBody,
          reset =
            !prior ||
            now - memory.seen > 200 ||
            distance(hand.anchor, previous.anchor) > hand.span * 2;
        memory.palmBody = {
          x: hand.anchor.x,
          y: hand.anchor.y,
          rx: hand.span * 0.28,
          ...
          vx: reset
            ? 0
            : clamp((hand.anchor.x - previous.anchor.x) / sampleDt, -350, 350),
          vy: reset
            ? 0
            : clamp((hand.anchor.y - previous.anchor.y) / sampleDt, -350, 350),
          omega: 0,
          reset,
        };
      ...
      const accepted =
        hand.heart && !hand.gun && !this.autoBlocked && this.autoReady.hearts;
      ...
      if (
        ...
        if (memory.gunSince < 0) {
          memory.gunSince = now;
          memory.gx = hand.tip.x;
          memory.gy = hand.tip.y;
          memory.gdx = hand.gunDirection.x;
          memory.gdy = hand.gunDirection.y;
        } else {
          memory.gx += (hand.tip.x - memory.gx) * alpha;
          memory.gy += (hand.tip.y - memory.gy) * alpha;
          const old = Math.atan2(memory.gdy, memory.gdx),
            target = Math.atan2(hand.gunDirection.y, hand.gunDirection.x);
          const angle =
            old +
            Math.atan2(Math.sin(target - old), Math.cos(target - old)) * alpha;
          memory.gdx = Math.cos(angle);
          memory.gdy = Math.sin(angle);
        }
      } else {
        memory.gunSince = -1;
      }
    ...
    if (this.autoBlocked) this.suspendAuto();
      ...
      const candidates: EmitterCandidate[] = hands.map((hand) => {
        ...
        const kind =
          hand.heart && !hand.gun && this.autoReady.hearts
            ? 'hearts'
            : hand.gun &&
                !hand.heart &&
                !hand.heartPossible &&
        ...
        return {
          ...
          confirmed:
            kind === 'hearts'
              ? m.heartConfirmed
              : kind === 'bubble' && m.gunSince >= 0 && now - m.gunSince >= 250,
        };
      });
    ...
    for (const [id, memory] of this.memories)
      if (now - memory.seen > 350) this.memories.delete(id);
  }
  private suspendAuto() {
    ...
    for (const m of this.memories.values()) {
      m.heartSince = -1;
      m.heartConfirmed = false;
      m.gunSince = -1;
    }
  }
  private emitHeart(memory: HandMemory, now: number) {
    const hand = memory.hand;
    const limit = this.autoConfetti ? 8 : HEART_CAPACITY;
    let active = 0;
    let heart: Heart | undefined;
    for (const h of this.hearts) {
      if (h.active) {
        active++;
      } else if (!heart) heart = h;
    }
    if (active >= limit) heart = undefined;
    // A full pool consumes this opportunity; it never creates a deferred burst.
    this.lastSharedHeart = now;
    if (!heart) {
      this.dropped.hearts++;
      return;
    }
    const scatter = heartBirthOffset(memory.heartSequence),
      emitterAngle = Math.atan2(memory.dy, memory.dx),
      angle = emitterAngle + scatter;
    const inherited = Math.hypot(memory.ovx, memory.ovy) * 0.25,
      inheritScale = inherited > 40 ? 40 / inherited : 0.999999;
    const prediction = Math.min(
      0.08,
      12 / Math.max(1, Math.hypot(memory.ovx, memory.ovy)),
    );
    Object.assign(heart, {
      active: true,
      x: memory.ox + memory.ovx * prediction,
      y: memory.oy + memory.ovy * prediction,
      age: 0,
      owner: hand.id,
      vx: Math.cos(angle) * 80 + memory.ovx * 0.25 * inheritScale,
      vy: Math.sin(angle) * 80 + memory.ovy * 0.25 * inheritScale,
      angle: angle + Math.PI / 2,
      emitterAngle,
      spreadAngle: scatter,
      appearanceAngle: heartAppearanceAngle(emitterAngle, scatter),
      birthOrder: this.emittedHearts,
      colorOrder: memory.heartSequence,
      size: heartBirthSize(this.width, this.height, this.emittedHearts),
      ...
      motionAngle: 0,
      angularVelocity: 0,
      contactBody: undefined,
      contactReleased: true,
    });
    this.emittedHearts++;
    memory.heartSequence++;
    this.lastHeartOwner = hand.id;
  }
  ...
  step(dt: number, now: number, elapsedSeconds = dt) {
    const portrait = this.height > this.width;
    if (this.autoBlocked) this.suspendAuto();
    const primary = !this.autoBlocked
      ? this.primaryEmitter.select(now)
      : undefined;
    if (primary?.kind === 'hearts' && this.autoReady.hearts) {
      ...
      if (
        ...
        m.heartConfirmed &&
        now - m.seen <= 200 &&
        now - m.directionSeen <= 200 &&
        now - this.lastSharedHeart >= AUTO_HEART_INTERVAL_MS
      )
        this.emitHeart(m, now);
    }
    ...
    for (const memory of bubbleMemories) {
      ...
      if (
        this.autoBlocked ||
        !this.autoReady.bubble ||
        now - this.lastSharedBubble < interval ||
        memory.hand.heart ||
      ...
      const prediction = Math.min(
        0.06,
        8 / Math.max(1, Math.hypot(memory.vx, memory.vy)),
      );
      Object.assign(bubble, {
        active: true,
        x: clamp(
          memory.gx + memory.vx * prediction + memory.gdx * (r + 3),
          r,
          this.width - r,
        ),
        y: clamp(
          memory.gy + memory.vy * prediction + memory.gdy * (r + 3),
          r + 8,
          this.height - r - 8,
        ),
        vx: Math.cos(angle) * speed + memory.vx * 0.08,
        vy: Math.sin(angle) * speed + memory.vy * 0.08,
        driftPhase: (this.emittedBubbles * 2.39996323) % (Math.PI * 2),
        driftRate: 1.1 + ((this.emittedBubbles * 0.41421356) % 1) * 0.8,
        driftSpeed:
          this.width *
          (0.045 + ((this.emittedBubbles * 0.7320508) % 1) * 0.035),
        riseSpeed:
          this.height * (0.13 + 0.07 * (1 - Math.min(1, targetR / 36))),
        r,
        targetR,
        age: 0,
        pop: -1,
    ...
    for (const heart of this.hearts) {
      ...
      if (
        ...
        heart.x = memory.ox + memory.ovx * prediction;
    ...
    for (const bubble of this.bubbles) {
      ...
      bubble.age += elapsedSeconds;
      bubble.squash = Math.max(0, (bubble.squash ?? 0) - dt);
      if (bubble.targetR) bubble.r = bubbleRadius(bubble.targetR, bubble.age);
      const lifetime = bubble.targetR ? BUBBLE_LIFETIME : 12;
      if (bubble.pop >= 0) {
        bubble.pop += elapsedSeconds;
      } else if (bubble.age >= lifetime) {
        this.popBubble(bubble);
        bubble.pop = bubble.age - lifetime;
      }
      if (bubble.pop >= 0) {
        if (bubble.pop >= 0.32) bubble.active = false;
        continue;
      }
      if (portrait) {
        // Relax toward individual drift velocities; keep palm impulses continuous.
        const blend = Math.min(1, bubble.age / 0.6);
        const drift =
          Math.sin(
            bubble.age * (bubble.driftRate ?? 1.4) + (bubble.driftPhase ?? 0),
          ) * (bubble.driftSpeed ?? this.width * 0.06);
        bubble.vx += (drift - bubble.vx) * blend * 1.4 * dt;
        bubble.vy +=
          (-(bubble.riseSpeed ?? this.height * 0.16) - bubble.vy) *
          blend *
          1.8 *
          dt;
      } else {
        bubble.vx += -bubble.vx * 0.55 * dt;
        bubble.vy += (-8 - bubble.vy * 0.55) * dt;
      }
      ...
      for (const memory of this.memories.values())
        if (memory.palmBody && now - memory.seen <= 200)
          collideHead(
            bubble,
            px,
            py,
      ...
      if (!portrait && this.head && now - this.head.timestamp <= 200)
        collideHead(
          bubble,
          px,
          py,
      ...
      if (portrait) {
        ...
        if (bubble.x > this.width - bubble.r) {
          bubble.x = this.width - bubble.r;
          bubble.vx = -Math.abs(bubble.vx) * 0.3;
        }
        if (bubble.y < bubble.r + 8) {
          bubble.y = bubble.r + 8;
          bubble.vy = Math.max(0, bubble.vy);
        }
        if (bubble.y > this.height - bubble.r - 8) {
          bubble.y = this.height - bubble.r - 8;
          bubble.vy = -Math.abs(bubble.vy) * 0.35;
        }
      }
      const finalSpeed = Math.hypot(bubble.vx, bubble.vy);
      if (finalSpeed > 420) {
        bubble.vx *= 420 / finalSpeed;
        bubble.vy *= 420 / finalSpeed;
      }
    }
  }
  get count() {
