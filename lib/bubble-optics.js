...
export function setBubbleUniforms(material,state,width,height,pixelRatio=1){
 const u=material.uniforms;u.uResolution.value.set(width*pixelRatio,height*pixelRatio);u.uTime.value=state.age;
 u.uSeed.value=state.birthId??0;u.uRadiusPx.value=state.r*pixelRatio;u.uBirthAlpha.value=Math.min(1,state.age/.10);
